<template>
  <div class="app-container">
    <p class="app-title">启发式规则Ⅳ-基于DappType的混币账户去匿名化界面</p>
    <p></p>
    <!-- Search input -->
    ...
    <el-table
      ...
      <el-table-column prop="dapp_count" label="Dapp数量" header-align="center">
        <template slot-scope="scope" >
          <div style="font-weight: bold; color: #000000; text-align: center;">{{ scope.row.dapp_count }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="dapp_list" label="相同资产" header-align="center">
        <template slot-scope="scope">
          <div style="font-weight: bold; color: #909399; text-align: center;">{{ scope.row.dapp_list }}</div>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import { getAccountDappList } from '@/api/GetHeuristicRulesⅣ_DappTypeLink'
...
export default {
  ...
  methods: {
    search() {
      ...
      getAccountDappList({ address: this.searchAddress }).then(response => {
