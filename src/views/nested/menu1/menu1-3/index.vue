<template>
  <div class="app-container">
    <p class="app-title">启发式规则③-基于ERC20交易的混币账户去匿名化界面</p>
    <p></p>
    <!-- Search input -->
    ...
    <el-table
      ...
      <el-table-column prop="link_list" label="交易哈希" header-align="center">
        <template slot-scope="scope">
          <div style="font-weight: bold; color: #909399; text-align: center;">{{ scope.row.hash }}</div>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
import { getERC20TransferAccount } from '@/api/GetHeuristicRulesⅢ_ERC20TransferLink'
...
export default {
  ...
  methods: {
    search() {
      ...
      getERC20TransferAccount({ address: this.searchAddress }).then(response => {
